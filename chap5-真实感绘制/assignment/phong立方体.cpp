...
// 输出顶点数组
float* getCubeVertice(int& verticeCount, float r = 1);

glm::mat4 model;
glm::mat4 view;
glm::mat4 projection;

glm::vec3 viewPos = glm::vec3(2.0f, 2.0f, 2.5f);
glm::vec3 viewFront = glm::normalize(glm::vec3(0.0f, 0.0f, 0.0f) - viewPos);
...
int main() {
    ...
    GLFWwindow* window =
        glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Phong立方体", NULL, NULL);
    if (window == NULL) {
    ...
    // 读取着色器
    // 着色器使用Phong光照模型对立方体着色，实现细节和伪代码类似
    writeShader(vertexShader, fragmentShader);
    Shader shader(vertexShader.c_str(), fragmentShader.c_str());

    // 通过函数生成一个立方体的顶点信息并将其设为绘制对象

    float* cubeVertice;
    int verticeCount;
    cubeVertice = getCubeVertice(verticeCount);

    GLuint VBO;
    glGenBuffers(1, &VBO);
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * verticeCount * 6, cubeVertice,
                 GL_STATIC_DRAW);
    GLuint VAO;
...
float* getCubeVertice(int& verticeCount, float r) {
    verticeCount = 36;
    float* result = new float[36 * 6];

    int index = 0;
    auto pushVertex = [&](float px, float py, float pz, float nx, float ny,
                          float nz) {
        result[index++] = px;
        result[index++] = py;
        result[index++] = pz;
