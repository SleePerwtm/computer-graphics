...
class DDADrawLine : public DrawLineBase {
    vector<int>& DrawLine(int x0, int y0, int x1, int y1);
};

vector<int>& DDADrawLine::DrawLine(int x0, int y0, int x1, int y1) {
    vector<int>* pIndexVec = new vector<int>();

    int delta_x = 0, delta_y = 0, abs_delta_x = 0, abs_delta_y = 0,
        max_delta = 0;
    ...
    max_delta = abs_delta_x > abs_delta_y ? abs_delta_x : abs_delta_y;

    // 如果两点为同一点，则返回
    if (max_delta == 0) {
        DrawPixel(x0, y0, *pIndexVec);
        return *pIndexVec;
    }
    ...
    x = x0;
    y = y0;
    dx = (float)delta_x / max_delta;
    dy = (float)delta_y / max_delta;

    // 画直线
    for (int i = 0; i <= max_delta; i++) {
        DrawPixel(x + 0.5f, y + 0.5f, *pIndexVec);
        x += dx;
        y += dy;
    }
...
int main() {
    ...
    GLFWwindow* window =
        glfwCreateWindow(WIDTH, HEIGHT, u8"例题 2.3.1-1", nullptr, nullptr);
    ...
    DrawLineBase* pDrawLine = new DDADrawLine();
